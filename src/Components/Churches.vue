                ...
                <div class="relative z-10 flex flex-col items-center">
                  ...
                  >
                    <svg
                      ...
                      :class="
                        selectedCategories.includes(stat.category) ? 'text-white' : 'text-[#004595]'
                      "
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
              ...
              <div v-if="false" class="space-y-3">
                ...
                <div class="flex flex-col items-center mb-3">
                  <div class="relative group">
                    ...
                    >
                      <img
                        v-if="editImagePreview || item.churchImages"
                        :src="editImagePreview || item.churchImages"
                        :alt="item.churchName"
                        class="w-full h-full object-cover"
                      />
                      <div
                ...
                <div class="space-y-1.5 text-sm text-gray-600">
                  <div class="flex items-start gap-2">
                    ...
                    <p class="text-sm text-gray-600 flex-1">
                      {{ item.churchAddress || 'No address provided' }}
                    </p>
                  </div>
                </div>
                ...
                <div class="space-y-3">
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-[#002147] mb-1.5"
                      >Church Name <span class="text-red-500">*</span></label
                    >
                    <input
                      v-model="newName"
                      type="text"
          ...
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all">
            ...
            <div class="p-6">
              <p class="text-gray-700 text-sm leading-relaxed mb-6">
                Are you sure you want to delete this church? This action cannot be undone and will
          ...
          >
            ...
            <div class="relative">
              <div v-if="detailsItem?.churchImages" class="w-full h-64 overflow-hidden">
                <img
                  :src="detailsItem.churchImages"
                  :alt="detailsItem.churchName"
                  class="w-full h-full object-cover"
                />
              </div>
              <div
                v-else
                class="w-full h-64 bg-linear-to-br from-[#002147] via-[#00397a] to-[#004595] flex items-center justify-center relative overflow-hidden"
              >
                <div
                  class="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full blur-3xl -mr-20 -mt-20"
                ></div>
                <div
                  class="absolute bottom-0 left-0 w-40 h-40 bg-white/10 rounded-full blur-3xl -ml-20 -mb-20"
                ></div>
                <svg
            ...
            <div v-else class="p-6 space-y-6">
              ...
              <div class="text-center border-b border-[#004595]/10 pb-4">
                <h2 class="text-3xl font-bold text-[#002147]">
                  {{ detailsItem?.churchName || 'N/A' }}
                </h2>
              </div>
                ...
                <div class="space-y-4">
                  ...
                  <div class="space-y-3">
                    <div class="flex items-start gap-3 p-3 bg-[#f3f1ee]/50 rounded-lg">
                      <svg
                        class="w-5 h-5 text-[#00397a] mt-0.5 shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      <div class="flex-1">
                        <p
                          class="text-xs font-semibold text-[#002147] uppercase tracking-wide mb-1"
                        >
                          Address
                        </p>
                        <p class="text-sm text-gray-700">
                          {{ detailsItem?.churchAddress || 'No address provided' }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3 p-3 bg-[#f3f1ee]/50 rounded-lg">
                      ...
                      <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-700">
                          {{ communityInfo?.name || 'N/A' }}, Butuan City
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="space-y-3">
                    <div class="flex items-start gap-3 p-3 bg-[#f3f1ee]/50 rounded-lg">
                      ...
                      <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-700">
                          {{ detailsItem?.leaderName || 'Not assigned' }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3 p-3 bg-[#f3f1ee]/50 rounded-lg">
                      ...
                      <div class="flex-1">
                        ...
                        <span
                          v-if="
                            detailsItem?.leaderRegistration === 'yes' ||
                            detailsItem?.leaderRegistration === true
                          "
                        ...
                        <span
                          v-else
                          class="inline-flex items-center gap-1.5 px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-semibold"
                        >
                          <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                            <path
                              fill-rule="evenodd"
...
onMounted(() => {
  ...
  if (selectedCategoriesQuery) {
    ...
    const cats = selectedCategoriesQuery
      .toString()
      .split(',')
      .map((cat) => cat.trim())
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      if (leaderName || newLeaderRegistration.value) {
        const { error: leaderError } = await supabase.from('ChurchLeader').insert([
          {
...
const saveEdit = async () => {
  ...
  try {
    ...
    if (existingLeader) {
      ...
      if (leaderName) {
        const { error: insertError } = await supabase.from('ChurchLeader').insert({
          fullname: leaderName,
          registration: leaderRegistered,
...
const deleteItem = async () => {
  ...
  try {
    ...
    const { error } = await supabase.from('Church').delete().eq('id', deleteId.value)
