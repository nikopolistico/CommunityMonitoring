							...
							<tr v-for="(item, index) in filteredItems" :key="item.id || index" class="hover:bg-gray-50 transition-colors">
								<td class="px-6 py-4">
									<div class="flex items-center gap-3">
										<div v-if="item.schoolImages" class="w-12 h-12 rounded-lg overflow-hidden shrink-0">
											<img :src="item.schoolImages" :alt="item.schoolName" class="w-full h-full object-cover" />
										</div>
				...
				<div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all">
					...
					<form @submit.prevent="addItem" class="p-6">
						<div class="space-y-4">
							...
							<div>
								<label class="block text-xs font-semibold text-[#002147] mb-1.5">Address <span class="text-red-500">*</span></label>
								<input
									v-model="newAddress"
									type="text"
							...
							<div class="grid gap-4 md:grid-cols-2">
								...
								<div class="rounded-lg border-2 border-[#004595]/10 bg-linear-to-br from-[#f3f1ee]/30 to-white px-4 py-3">
									<div class="flex items-center gap-2 mb-1.5">
										...
										<p class="text-xs uppercase tracking-wider text-gray-500 font-semibold">School Type</p>
									</div>
									<p class="font-bold text-[#002147] text-sm">{{ detailsItem?.category || 'Not provided' }}</p>
								</div>
